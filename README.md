...
We ended up using Supabase and FastAPI on Render instead of GCP, since GCP required a billing account and none of us could commit to that for a lab. Cloud Run became FastAPI, Pub/Sub became a Postgres vote_queue table, and Firestore became a votes table, but the distributed concepts stayed the same. What hit me hardest was how different distributed execution felt from sequential execution. In a normal script, vote generation, transmission, and processing happen in one blocking call, but here the edge node, API, and worker each ran on their own rhythm with the queue being the only thing coordinating them. As we added more edge nodes and ran the duplicate-send test, end-to-end latency stretched from one or two seconds under low load to noticeably longer as the pending count climbed, which made it obvious that throughput was bottlenecked by the worker's polling, not by ingestion, since the API stayed responsive the whole time. The hardest parts were the small painful ones, like figuring out the Supabase service-role key, getting the API and worker to actually integrate cleanly through the queue, and debugging asynchronous components when a worker failure threw no exception and just silently stopped writing to the database, forcing me to hunt through logs and SQL queries to find where messages were stuck. The worker-failure test was when it really clicked. The API kept returning 200 OK while the worker was dead, the queue buffered everything, and on restart the worker drained the backlog in about 15 seconds with no manual recovery. The duplicate-send test showed eventual consistency through the user_id_poll_id idempotency key. Distribution clearly helped when things failed, but for a small system the communication overhead of HTTP plus a queue plus a separate worker, along with Render's cold starts, was honestly slower than a single script would have been. The real takeaway is that distribution isn't about speed, it's about not falling over when one piece dies.

### 5.2 Member 2 — Francis Adrian Esteban

This project provided us with meaningful insights into teamwork, patience, and collaborative problem-solving. Working as a group helped us understand the importance of clear communication and coordination, especially when tackling complex technical tasks. We learned that effective teamwork is not just about dividing responsibilities, but also about supporting each other through challenges and setbacks. Patience became essential as we encountered unexpected issues that required time and careful thought to resolve.
One of the biggest challenges we faced was understanding how distributed systems function. Unlike traditional programming, where operations run in a predictable sequence, distributed systems involve multiple processes that may execute independently and out of order. There were several moments when the system appeared to be working correctly, yet some votes were not being processed as expected. These issues pushed us to dig deeper by examining logs, tracing message flows, and carefully analyzing how different components interacted with one another to identify the root cause.
